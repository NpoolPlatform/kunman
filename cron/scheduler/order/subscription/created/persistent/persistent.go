...
func (p *handler) Update(ctx context.Context, order interface{}, reward, notif, done chan interface{}) error {
	_order, ok := order.(*types.PersistentSubscriptionOrder)
	if !ok {
		return fmt.Errorf("invalid subscriptionorder")
	}
	...
	handler, err := subscriptionordermw.NewHandler(
		...
		subscriptionordermw.WithOrderState(ordertypes.OrderState_OrderStateWaitPayment.Enum(), true),
	)
	if err != nil {
		return err
	}
