...
func init() {
	...
	err := register.RegisteAbortFuncErr(plugin.CoinType_CoinTypebitcoin, btc.TxFailErr)
	if err != nil {
		panic(err)
	}

	err = register.RegisteAbortFuncErr(plugin.CoinType_CoinTypetbitcoin, btc.TxFailErr)
	if err != nil {
		panic(err)
	}
}
