...
func (*AppUserControl) scanValues(columns []string) ([]any, error) {
...
func (auc *AppUserControl) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		default:
			auc.selectValues.Set(columns[i], values[i])
		}
	}
	return nil
}

// Value returns the ent.Value that was dynamically selected and assigned to the AppUserControl.
// This includes values selected through modifiers, order, etc.
func (auc *AppUserControl) Value(name string) (ent.Value, error) {
	return auc.selectValues.Get(name)
}
...
func (auc *AppUserControl) Update() *AppUserControlUpdateOne {
	return NewAppUserControlClient(auc.config).UpdateOne(auc)
}
...
func (auc *AppUserControl) Unwrap() *AppUserControl {
	...
	if !ok {
		panic("generated: AppUserControl is not a transactional entity")
	}
