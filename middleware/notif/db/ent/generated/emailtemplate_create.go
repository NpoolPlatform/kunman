...
func (etc *EmailTemplateCreate) check() error {
	if _, ok := etc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`generated: missing required field "EmailTemplate.created_at"`)}
	}
	if _, ok := etc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`generated: missing required field "EmailTemplate.updated_at"`)}
	}
	if _, ok := etc.mutation.DeletedAt(); !ok {
		return &ValidationError{Name: "deleted_at", err: errors.New(`generated: missing required field "EmailTemplate.deleted_at"`)}
	}
	if _, ok := etc.mutation.EntID(); !ok {
		return &ValidationError{Name: "ent_id", err: errors.New(`generated: missing required field "EmailTemplate.ent_id"`)}
	}
	if _, ok := etc.mutation.AppID(); !ok {
		return &ValidationError{Name: "app_id", err: errors.New(`generated: missing required field "EmailTemplate.app_id"`)}
	}
	if _, ok := etc.mutation.LangID(); !ok {
		return &ValidationError{Name: "lang_id", err: errors.New(`generated: missing required field "EmailTemplate.lang_id"`)}
	}
	if _, ok := etc.mutation.DefaultToUsername(); !ok {
		return &ValidationError{Name: "default_to_username", err: errors.New(`generated: missing required field "EmailTemplate.default_to_username"`)}
	}
	return nil
}
...
func (etc *EmailTemplateCreate) createSpec() (*EmailTemplate, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(emailtemplate.Table, sqlgraph.NewFieldSpec(emailtemplate.FieldID, field.TypeUint32))
	)
...
func (u *EmailTemplateUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("generated: missing options for EmailTemplateCreate.OnConflict")
	}
...
func (u *EmailTemplateUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("generated: OnConflict was set for builder %d. Set it on the EmailTemplateCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("generated: missing options for EmailTemplateCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
