...
func (*PaymentBalanceLock) scanValues(columns []string) ([]any, error) {
...
func (pbl *PaymentBalanceLock) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		default:
			pbl.selectValues.Set(columns[i], values[i])
		}
	}
	return nil
}

// Value returns the ent.Value that was dynamically selected and assigned to the PaymentBalanceLock.
// This includes values selected through modifiers, order, etc.
func (pbl *PaymentBalanceLock) Value(name string) (ent.Value, error) {
	return pbl.selectValues.Get(name)
}
...
func (pbl *PaymentBalanceLock) Update() *PaymentBalanceLockUpdateOne {
	return NewPaymentBalanceLockClient(pbl.config).UpdateOne(pbl)
}
...
func (pbl *PaymentBalanceLock) Unwrap() *PaymentBalanceLock {
	...
	if !ok {
		panic("generated: PaymentBalanceLock is not a transactional entity")
	}
