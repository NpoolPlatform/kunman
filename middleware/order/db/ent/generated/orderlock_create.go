...
func (olc *OrderLockCreate) defaults() {
...
func (olc *OrderLockCreate) check() error {
	if _, ok := olc.mutation.EntID(); !ok {
		return &ValidationError{Name: "ent_id", err: errors.New(`generated: missing required field "OrderLock.ent_id"`)}
	}
	if _, ok := olc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`generated: missing required field "OrderLock.created_at"`)}
	}
	if _, ok := olc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`generated: missing required field "OrderLock.updated_at"`)}
	}
	if _, ok := olc.mutation.DeletedAt(); !ok {
		return &ValidationError{Name: "deleted_at", err: errors.New(`generated: missing required field "OrderLock.deleted_at"`)}
	}
	return nil
}
...
func (olc *OrderLockCreate) createSpec() (*OrderLock, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(orderlock.Table, sqlgraph.NewFieldSpec(orderlock.FieldID, field.TypeUint32))
	)
...
func (u *OrderLockUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("generated: missing options for OrderLockCreate.OnConflict")
	}
...
func (u *OrderLockUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("generated: OnConflict was set for builder %d. Set it on the OrderLockCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("generated: missing options for OrderLockCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
