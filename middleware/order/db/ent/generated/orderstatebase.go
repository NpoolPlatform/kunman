...
func (*OrderStateBase) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case orderstatebase.FieldID, orderstatebase.FieldStartAt, orderstatebase.FieldLastBenefitAt:
...
func (osb *OrderStateBase) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		default:
			osb.selectValues.Set(columns[i], values[i])
		}
	}
	return nil
}

// Value returns the ent.Value that was dynamically selected and assigned to the OrderStateBase.
// This includes values selected through modifiers, order, etc.
func (osb *OrderStateBase) Value(name string) (ent.Value, error) {
	return osb.selectValues.Get(name)
}
...
func (osb *OrderStateBase) Update() *OrderStateBaseUpdateOne {
	return NewOrderStateBaseClient(osb.config).UpdateOne(osb)
}
...
func (osb *OrderStateBase) Unwrap() *OrderStateBase {
	...
	if !ok {
		panic("generated: OrderStateBase is not a transactional entity")
	}
