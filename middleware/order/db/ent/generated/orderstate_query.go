...
func (osq *OrderStateQuery) Order(o ...orderstate.OrderOption) *OrderStateQuery {
...
func (osq *OrderStateQuery) All(ctx context.Context) ([]*OrderState, error) {
	...
	qr := querierAll[[]*OrderState, *OrderStateQuery]()
	return withInterceptors[[]*OrderState](ctx, osq, qr, osq.inters)
}
...
func (osq *OrderStateQuery) IDs(ctx context.Context) (ids []uint32, err error) {
...
func (osq *OrderStateQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, osq, querierCount[*OrderStateQuery](), osq.inters)
}
...
func (osq *OrderStateQuery) Clone() *OrderStateQuery {
	...
	return &OrderStateQuery{
		config:     osq.config,
		ctx:        osq.ctx.Clone(),
		order:      append([]orderstate.OrderOption{}, osq.order...),
...
func (osq *OrderStateQuery) GroupBy(field string, fields ...string) *OrderStateGroupBy {
	...
	grbuild := &OrderStateGroupBy{build: osq}
...
//
//	client.OrderState.Query().
//		Select(orderstate.FieldEntID).
//		Scan(ctx, &v)
func (osq *OrderStateQuery) Select(fields ...string) *OrderStateSelect {
	osq.ctx.Fields = append(osq.ctx.Fields, fields...)
	sbuild := &OrderStateSelect{OrderStateQuery: osq}
	sbuild.label = orderstate.Label
	sbuild.flds, sbuild.scan = &osq.ctx.Fields, sbuild.Scan
	return sbuild
}

// Aggregate returns a OrderStateSelect configured with the given aggregations.
func (osq *OrderStateQuery) Aggregate(fns ...AggregateFunc) *OrderStateSelect {
...
func (osq *OrderStateQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(orderstate.Table, orderstate.Columns, sqlgraph.NewFieldSpec(orderstate.FieldID, field.TypeUint32))
	_spec.From = osq.sql
...
func (osgb *OrderStateGroupBy) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, osgb.build.ctx, ent.OpQueryGroupBy)
	if err := osgb.build.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*OrderStateQuery, *OrderStateGroupBy](ctx, osgb.build, osgb, osgb.build.inters, v)
}

func (osgb *OrderStateGroupBy) sqlScan(ctx context.Context, root *OrderStateQuery, v any) error {
...
func (oss *OrderStateSelect) Aggregate(fns ...AggregateFunc) *OrderStateSelect {
	oss.fns = append(oss.fns, fns...)
	return oss
}

// Scan applies the selector query and scans the result into the given value.
func (oss *OrderStateSelect) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, oss.ctx, ent.OpQuerySelect)
	if err := oss.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*OrderStateQuery, *OrderStateSelect](ctx, oss.OrderStateQuery, oss, oss.inters, v)
}

func (oss *OrderStateSelect) sqlScan(ctx context.Context, root *OrderStateQuery, v any) error {
