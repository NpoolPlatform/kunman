...
func (foc *FeeOrderCreate) defaults() {
...
func (foc *FeeOrderCreate) check() error {
	if _, ok := foc.mutation.EntID(); !ok {
		return &ValidationError{Name: "ent_id", err: errors.New(`generated: missing required field "FeeOrder.ent_id"`)}
	}
	if _, ok := foc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`generated: missing required field "FeeOrder.created_at"`)}
	}
	if _, ok := foc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`generated: missing required field "FeeOrder.updated_at"`)}
	}
	if _, ok := foc.mutation.DeletedAt(); !ok {
		return &ValidationError{Name: "deleted_at", err: errors.New(`generated: missing required field "FeeOrder.deleted_at"`)}
	}
	return nil
}
...
func (foc *FeeOrderCreate) createSpec() (*FeeOrder, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(feeorder.Table, sqlgraph.NewFieldSpec(feeorder.FieldID, field.TypeUint32))
	)
...
func (u *FeeOrderUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("generated: missing options for FeeOrderCreate.OnConflict")
	}
...
func (u *FeeOrderUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("generated: OnConflict was set for builder %d. Set it on the FeeOrderCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("generated: missing options for FeeOrderCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
