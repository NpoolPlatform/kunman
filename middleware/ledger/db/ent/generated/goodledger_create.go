...
func (glc *GoodLedgerCreate) defaults() {
...
func (glc *GoodLedgerCreate) check() error {
	if _, ok := glc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`generated: missing required field "GoodLedger.created_at"`)}
	}
	if _, ok := glc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`generated: missing required field "GoodLedger.updated_at"`)}
	}
	if _, ok := glc.mutation.DeletedAt(); !ok {
		return &ValidationError{Name: "deleted_at", err: errors.New(`generated: missing required field "GoodLedger.deleted_at"`)}
	}
	if _, ok := glc.mutation.EntID(); !ok {
		return &ValidationError{Name: "ent_id", err: errors.New(`generated: missing required field "GoodLedger.ent_id"`)}
	}
	return nil
}
...
func (glc *GoodLedgerCreate) createSpec() (*GoodLedger, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(goodledger.Table, sqlgraph.NewFieldSpec(goodledger.FieldID, field.TypeUint32))
	)
...
func (u *GoodLedgerUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("generated: missing options for GoodLedgerCreate.OnConflict")
	}
...
func (u *GoodLedgerUpsertBulk) Exec(ctx context.Context) error {
	...
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("generated: OnConflict was set for builder %d. Set it on the GoodLedgerCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("generated: missing options for GoodLedgerCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}
