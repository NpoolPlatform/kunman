...
func (llu *LedgerLockUpdate) defaults() {
...
func (llu *LedgerLockUpdate) Modify(modifiers ...func(u *sql.UpdateBuilder)) *LedgerLockUpdate {
	llu.modifiers = append(llu.modifiers, modifiers...)
	return llu
}

func (llu *LedgerLockUpdate) sqlSave(ctx context.Context) (n int, err error) {
	_spec := sqlgraph.NewUpdateSpec(ledgerlock.Table, ledgerlock.Columns, sqlgraph.NewFieldSpec(ledgerlock.FieldID, field.TypeUint32))
	...
	if value, ok := llu.mutation.AddedCreatedAt(); ok {
		_spec.AddField(ledgerlock.FieldCreatedAt, field.TypeUint32, value)
	}
	...
	if value, ok := llu.mutation.AddedUpdatedAt(); ok {
		_spec.AddField(ledgerlock.FieldUpdatedAt, field.TypeUint32, value)
	}
	...
	if value, ok := llu.mutation.AddedDeletedAt(); ok {
		_spec.AddField(ledgerlock.FieldDeletedAt, field.TypeUint32, value)
	}
	...
	if llu.mutation.LedgerIDCleared() {
		_spec.ClearField(ledgerlock.FieldLedgerID, field.TypeUUID)
	}
	...
	if llu.mutation.StatementIDCleared() {
		_spec.ClearField(ledgerlock.FieldStatementID, field.TypeUUID)
	}
	...
	if value, ok := llu.mutation.AddedAmount(); ok {
		_spec.AddField(ledgerlock.FieldAmount, field.TypeFloat64, value)
	}
	if llu.mutation.AmountCleared() {
		_spec.ClearField(ledgerlock.FieldAmount, field.TypeFloat64)
	}
	...
	if llu.mutation.LockStateCleared() {
		_spec.ClearField(ledgerlock.FieldLockState, field.TypeString)
	}
	...
	if llu.mutation.ExLockIDCleared() {
		_spec.ClearField(ledgerlock.FieldExLockID, field.TypeUUID)
	}
...
// Where appends a list predicates to the LedgerLockUpdate builder.
func (lluo *LedgerLockUpdateOne) Where(ps ...predicate.LedgerLock) *LedgerLockUpdateOne {
...
// defaults sets the default values of the builder before save.
func (lluo *LedgerLockUpdateOne) defaults() {
...
// Modify adds a statement modifier for attaching custom logic to the UPDATE statement.
func (lluo *LedgerLockUpdateOne) Modify(modifiers ...func(u *sql.UpdateBuilder)) *LedgerLockUpdateOne {
...
func (lluo *LedgerLockUpdateOne) sqlSave(ctx context.Context) (_node *LedgerLock, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`generated: missing "LedgerLock.id" for update`)}
	}
