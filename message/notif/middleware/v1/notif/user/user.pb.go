// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.33.0
// 	protoc        v3.18.1
// source: notif/middleware/v1/notif/user/user.proto

package user

import (
	reflect "reflect"
	sync "sync"

	v1 "github.com/NpoolPlatform/kunman/message/basetypes/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type NotifUser struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// @inject_tag: sql:"id"
	ID uint32 `protobuf:"varint,9,opt,name=ID,proto3" json:"ID,omitempty" sql:"id"`
	// @inject_tag: sql:"ent_id"
	EntID string `protobuf:"bytes,10,opt,name=EntID,proto3" json:"EntID,omitempty" sql:"ent_id"`
	// @inject_tag: sql:"app_id"
	AppID string `protobuf:"bytes,20,opt,name=AppID,proto3" json:"AppID,omitempty" sql:"app_id"`
	// @inject_tag: sql:"user_id"
	UserID string `protobuf:"bytes,30,opt,name=UserID,proto3" json:"UserID,omitempty" sql:"user_id"`
	// @inject_tag: sql:"event_type"
	EventTypeStr string     `protobuf:"bytes,40,opt,name=EventTypeStr,proto3" json:"EventTypeStr,omitempty" sql:"event_type"`
	EventType    v1.UsedFor `protobuf:"varint,50,opt,name=EventType,proto3,enum=basetypes.v1.UsedFor" json:"EventType,omitempty"`
	// @inject_tag: sql:"created_at"
	CreatedAt uint32 `protobuf:"varint,60,opt,name=CreatedAt,proto3" json:"CreatedAt,omitempty" sql:"created_at"`
	// @inject_tag: sql:"updated_at"
	UpdatedAt uint32 `protobuf:"varint,70,opt,name=UpdatedAt,proto3" json:"UpdatedAt,omitempty" sql:"updated_at"`
}

func (x *NotifUser) Reset() {
	*x = NotifUser{}
	if protoimpl.UnsafeEnabled {
		mi := &file_notif_middleware_v1_notif_user_user_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *NotifUser) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NotifUser) ProtoMessage() {}

func (x *NotifUser) ProtoReflect() protoreflect.Message {
	mi := &file_notif_middleware_v1_notif_user_user_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NotifUser.ProtoReflect.Descriptor instead.
func (*NotifUser) Descriptor() ([]byte, []int) {
	return file_notif_middleware_v1_notif_user_user_proto_rawDescGZIP(), []int{0}
}

func (x *NotifUser) GetID() uint32 {
	if x != nil {
		return x.ID
	}
	return 0
}

func (x *NotifUser) GetEntID() string {
	if x != nil {
		return x.EntID
	}
	return ""
}

func (x *NotifUser) GetAppID() string {
	if x != nil {
		return x.AppID
	}
	return ""
}

func (x *NotifUser) GetUserID() string {
	if x != nil {
		return x.UserID
	}
	return ""
}

func (x *NotifUser) GetEventTypeStr() string {
	if x != nil {
		return x.EventTypeStr
	}
	return ""
}

func (x *NotifUser) GetEventType() v1.UsedFor {
	if x != nil {
		return x.EventType
	}
	return v1.UsedFor(0)
}

func (x *NotifUser) GetCreatedAt() uint32 {
	if x != nil {
		return x.CreatedAt
	}
	return 0
}

func (x *NotifUser) GetUpdatedAt() uint32 {
	if x != nil {
		return x.UpdatedAt
	}
	return 0
}

type NotifUserReq struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID        *uint32     `protobuf:"varint,9,opt,name=ID,proto3,oneof" json:"ID,omitempty"`
	EntID     *string     `protobuf:"bytes,10,opt,name=EntID,proto3,oneof" json:"EntID,omitempty"`
	AppID     *string     `protobuf:"bytes,20,opt,name=AppID,proto3,oneof" json:"AppID,omitempty"`
	UserID    *string     `protobuf:"bytes,30,opt,name=UserID,proto3,oneof" json:"UserID,omitempty"`
	EventType *v1.UsedFor `protobuf:"varint,40,opt,name=EventType,proto3,enum=basetypes.v1.UsedFor,oneof" json:"EventType,omitempty"`
}

func (x *NotifUserReq) Reset() {
	*x = NotifUserReq{}
	if protoimpl.UnsafeEnabled {
		mi := &file_notif_middleware_v1_notif_user_user_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *NotifUserReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NotifUserReq) ProtoMessage() {}

func (x *NotifUserReq) ProtoReflect() protoreflect.Message {
	mi := &file_notif_middleware_v1_notif_user_user_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NotifUserReq.ProtoReflect.Descriptor instead.
func (*NotifUserReq) Descriptor() ([]byte, []int) {
	return file_notif_middleware_v1_notif_user_user_proto_rawDescGZIP(), []int{1}
}

func (x *NotifUserReq) GetID() uint32 {
	if x != nil && x.ID != nil {
		return *x.ID
	}
	return 0
}

func (x *NotifUserReq) GetEntID() string {
	if x != nil && x.EntID != nil {
		return *x.EntID
	}
	return ""
}

func (x *NotifUserReq) GetAppID() string {
	if x != nil && x.AppID != nil {
		return *x.AppID
	}
	return ""
}

func (x *NotifUserReq) GetUserID() string {
	if x != nil && x.UserID != nil {
		return *x.UserID
	}
	return ""
}

func (x *NotifUserReq) GetEventType() v1.UsedFor {
	if x != nil && x.EventType != nil {
		return *x.EventType
	}
	return v1.UsedFor(0)
}

type Conds struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID        *v1.Uint32Val      `protobuf:"bytes,9,opt,name=ID,proto3,oneof" json:"ID,omitempty"`
	EntID     *v1.StringVal      `protobuf:"bytes,10,opt,name=EntID,proto3,oneof" json:"EntID,omitempty"`
	EntIDs    *v1.StringSliceVal `protobuf:"bytes,20,opt,name=EntIDs,proto3,oneof" json:"EntIDs,omitempty"`
	AppID     *v1.StringVal      `protobuf:"bytes,30,opt,name=AppID,proto3,oneof" json:"AppID,omitempty"`
	UserID    *v1.StringVal      `protobuf:"bytes,40,opt,name=UserID,proto3,oneof" json:"UserID,omitempty"`
	EventType *v1.Uint32Val      `protobuf:"bytes,50,opt,name=EventType,proto3,oneof" json:"EventType,omitempty"`
}

func (x *Conds) Reset() {
	*x = Conds{}
	if protoimpl.UnsafeEnabled {
		mi := &file_notif_middleware_v1_notif_user_user_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Conds) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Conds) ProtoMessage() {}

func (x *Conds) ProtoReflect() protoreflect.Message {
	mi := &file_notif_middleware_v1_notif_user_user_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Conds.ProtoReflect.Descriptor instead.
func (*Conds) Descriptor() ([]byte, []int) {
	return file_notif_middleware_v1_notif_user_user_proto_rawDescGZIP(), []int{2}
}

func (x *Conds) GetID() *v1.Uint32Val {
	if x != nil {
		return x.ID
	}
	return nil
}

func (x *Conds) GetEntID() *v1.StringVal {
	if x != nil {
		return x.EntID
	}
	return nil
}

func (x *Conds) GetEntIDs() *v1.StringSliceVal {
	if x != nil {
		return x.EntIDs
	}
	return nil
}

func (x *Conds) GetAppID() *v1.StringVal {
	if x != nil {
		return x.AppID
	}
	return nil
}

func (x *Conds) GetUserID() *v1.StringVal {
	if x != nil {
		return x.UserID
	}
	return nil
}

func (x *Conds) GetEventType() *v1.Uint32Val {
	if x != nil {
		return x.EventType
	}
	return nil
}

var File_notif_middleware_v1_notif_user_user_proto protoreflect.FileDescriptor

var file_notif_middleware_v1_notif_user_user_proto_rawDesc = []byte{
	,
}

var (
	file_notif_middleware_v1_notif_user_user_proto_rawDescOnce sync.Once
	file_notif_middleware_v1_notif_user_user_proto_rawDescData = file_notif_middleware_v1_notif_user_user_proto_rawDesc
)

func file_notif_middleware_v1_notif_user_user_proto_rawDescGZIP() []byte {
	file_notif_middleware_v1_notif_user_user_proto_rawDescOnce.Do(func() {
		file_notif_middleware_v1_notif_user_user_proto_rawDescData = protoimpl.X.CompressGZIP(file_notif_middleware_v1_notif_user_user_proto_rawDescData)
	})
	return file_notif_middleware_v1_notif_user_user_proto_rawDescData
}

var (
	file_notif_middleware_v1_notif_user_user_proto_msgTypes = make([]protoimpl.MessageInfo, 3)
	file_notif_middleware_v1_notif_user_user_proto_goTypes  = []interface{}{
		(*NotifUser)(nil),         // 0: notif.middleware.notif.user.v1.NotifUser
		(*NotifUserReq)(nil),      // 1: notif.middleware.notif.user.v1.NotifUserReq
		(*Conds)(nil),             // 2: notif.middleware.notif.user.v1.Conds
		(v1.UsedFor)(0),           // 3: basetypes.v1.UsedFor
		(*v1.Uint32Val)(nil),      // 4: basetypes.v1.Uint32Val
		(*v1.StringVal)(nil),      // 5: basetypes.v1.StringVal
		(*v1.StringSliceVal)(nil), // 6: basetypes.v1.StringSliceVal
	}
)
var file_notif_middleware_v1_notif_user_user_proto_depIdxs = []int32{
	3, // 0: notif.middleware.notif.user.v1.NotifUser.EventType:type_name -> basetypes.v1.UsedFor
	3, // 1: notif.middleware.notif.user.v1.NotifUserReq.EventType:type_name -> basetypes.v1.UsedFor
	4, // 2: notif.middleware.notif.user.v1.Conds.ID:type_name -> basetypes.v1.Uint32Val
	5, // 3: notif.middleware.notif.user.v1.Conds.EntID:type_name -> basetypes.v1.StringVal
	6, // 4: notif.middleware.notif.user.v1.Conds.EntIDs:type_name -> basetypes.v1.StringSliceVal
	5, // 5: notif.middleware.notif.user.v1.Conds.AppID:type_name -> basetypes.v1.StringVal
	5, // 6: notif.middleware.notif.user.v1.Conds.UserID:type_name -> basetypes.v1.StringVal
	4, // 7: notif.middleware.notif.user.v1.Conds.EventType:type_name -> basetypes.v1.Uint32Val
	8, // [8:8] is the sub-list for method output_type
	8, // [8:8] is the sub-list for method input_type
	8, // [8:8] is the sub-list for extension type_name
	8, // [8:8] is the sub-list for extension extendee
	0, // [0:8] is the sub-list for field type_name
}

func init() { file_notif_middleware_v1_notif_user_user_proto_init() }
func file_notif_middleware_v1_notif_user_user_proto_init() {
	if File_notif_middleware_v1_notif_user_user_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_notif_middleware_v1_notif_user_user_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*NotifUser); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_notif_middleware_v1_notif_user_user_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*NotifUserReq); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_notif_middleware_v1_notif_user_user_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Conds); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	file_notif_middleware_v1_notif_user_user_proto_msgTypes[1].OneofWrappers = []interface{}{}
	file_notif_middleware_v1_notif_user_user_proto_msgTypes[2].OneofWrappers = []interface{}{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_notif_middleware_v1_notif_user_user_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   3,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_notif_middleware_v1_notif_user_user_proto_goTypes,
		DependencyIndexes: file_notif_middleware_v1_notif_user_user_proto_depIdxs,
		MessageInfos:      file_notif_middleware_v1_notif_user_user_proto_msgTypes,
	}.Build()
	File_notif_middleware_v1_notif_user_user_proto = out.File
	file_notif_middleware_v1_notif_user_user_proto_rawDesc = nil
	file_notif_middleware_v1_notif_user_user_proto_goTypes = nil
	file_notif_middleware_v1_notif_user_user_proto_depIdxs = nil
}
