...
func (s *Server) AdminGetTopMostGoodConstraints(ctx context.Context, in *npool.AdminGetTopMostGoodConstraintsRequest) (*npool.AdminGetTopMostGoodConstraintsResponse, error) {
	...
	if err != nil {
		logger.Sugar().Errorw(
			"AdminGetTopMostGoodConstraints",
			"In", in,
			"Error", err,
		)
		return &npool.AdminGetTopMostGoodConstraintsResponse{}, status.Error(codes.Aborted, err.Error())
	}
	...
	return &npool.AdminGetTopMostGoodConstraintsResponse{
		Infos: infos,
		Total: total,
	}, nil
}
